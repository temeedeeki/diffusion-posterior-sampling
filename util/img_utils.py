...
def prepare_im(load_dir, image_size: int, device):
...
def fold_unfold(img_t, kernel: int, stride: int):
...
def reshape_patch(x, crop_size: int = 128, dim_size: int = 3):
    x = x.transpose(0, 2).squeeze()  # [9, 3*(128**2)]
    x = x.view(dim_size**2, 3, crop_size, crop_size)
    return x


def reshape_patch_back(x, crop_size: int = 128, dim_size: int = 3):
...
class Unfolder:
    def __init__(self, img_size: int = 256, crop_size: int = 128, stride: int = 64):
